...
class MainWindow(QWidget):
    ...
    def _connect_to_db(self):
        self.conn = psycopg2.connect(database="subjects",
                                         user="postgres",
                                         password="3012",
                                         host="localhost",
                                         port="5432")
    ...
    def _create_schedule_table(self):
        ...
        self.schedule_table.setHorizontalHeaderLabels(["id", "day", "subject", "room_numb", "start_time", "end_time", "week"])
    ...
    def _add_to_timetable(self, num):
        ...
        try:
            ...
            self.cursor.execute(f"INSERT INTO timetable({columns[0]}, {columns[1]}, {columns[2]}, {columns[3]}, {columns[4]}, {columns[5]}, {columns[6]}) values('{row[0]}', '{row[1]}', '{row[2]}', '{row[3]}', '{row[4]}', '{row[5]}', '{row[6]}')")
            self.conn.commit()
        except:
    ...
    def _add_to_teacherstable(self, num):
        ...
        try:
            columns = ["id", "full_name", "subject"]
            self.cursor.execute(f"INSERT INTO teacher({columns[0]}, {columns[1]}, {columns[2]}) values('{row[0]}', '{row[1]}', '{row[2]}')")
            self.conn.commit()
        except:
            QMessageBox.about(self, "Error", "Enter all fields")
